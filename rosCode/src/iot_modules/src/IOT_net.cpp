...
int main(int argc, char **argv)
{
    ...
    while(ros::ok())
    {
        ...
        if((module_adderss_S==1)&&(module_adderss_R==1)&&(place==1))  //判断发送的地址和接收的模块地址是否相同
        {
            if(module_data1.d == 1)
            {
                words.data="关闭风扇"+ToString(module_data1.d);
            ...
            {
                words.data="开启风扇"+ToString(module_data1.d);
            ...
            if(module_data2.d == 1)
            {
                words.data="关闭电灯"+ToString(module_data2.d);
                ROS_INFO("close the light.");
                pub_words.publish(words);
                serial_write(1,2,0,2,0,0);  //地址1 不上传信息  关灯
            }
            else if(module_data2.d == 2)
            {
                words.data="开启电灯"+ToString(module_data2.d);
                ROS_INFO("open the light.");
                pub_words.publish(words);
                serial_write(1,2,0,1,0,0);
            }

            else
               ROS_INFO("the receive data is not right.");

            module_adderss_S = 9; //停止读取串口数据
            module_adderss_R = 0;  
            //place=0;
            serial_write(9,1,0,1,0,0);         
        sleep(2);
	}
       
        else if((module_adderss_S==9)&&(module_adderss_R==9)&&(place==1))  //判断发送的地址和接收的模块地址是否相同
        {
            if(module_data2.d == 1)
            {
                ...
                pub_words.publish(words);
                serial_write(9,2,0,2,0,0);  //地址1 不上传信息  
            }
            else if(module_data2.d == 2)
            {
        ...
        {
            words.data="已到吃药时间，该吃药了。";//"北京钢铁侠科技拥有一流的研发团队，提供仿人机器人整体解决方案，同时在机器人关键零部件方面做到了完全自主研发，大大降低了成本，提高了竞争力。";
            ROS_INFO("received the sensor data.");
            pub_words.publish(words);       
            module_adderss_S = 0; //停止读取串口数据
            module_adderss_R=0;
        ...
        {
            words.data="大家好，欢迎大家来到北京钢铁侠科技有限公司，我是助老服务机器人，我可以帮助管理家庭环境的安全。";
        ...
        {
            words.data="北京钢铁侠科技拥有一流的研发团队，提供仿人机器人整体解决方案，同时在机器人关键零部件方面做到了完全自主研发，大大降低了成本，提高了竞争力。";
            ROS_INFO("working in the sim...");
            pub_words.publish(words);       
            module_adderss_S = 0; 
